...
export default function EditSubCategoryScreen() {
  ...
  const initialPhotoUrl = searchParams.photoUrl as string;
  ...
  const [imageUri, setImageUri] = useState<string | null>(initialPhotoUrl || null);
  const [imageError, setImageError] = useState('');

  const validateName = (value: string) => {
    const trimmed = value.trim();
    if (!trimmed) return 'Sub Category name is required';
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Check if image is new (local URI) or existing (remote URL)
      ...
      if (isNewImage) {
        if (Platform.OS === 'web') {
          ...
          const file = new File([blob], 'subcategory.jpg', { type: 'image/jpeg' });
        ...
        // Assume backend needs existingPhotoUrl when no new file is provided
        formData.append('existingPhotoUrl', imageUri);
      }

      console.log(`🚀 Sending Multipart PUT to /subcategories/${id}...`);
      ...
      console.log('✅ Updated Subcategory:', JSON.stringify(res.data));
      router.back();

    } catch (e: any) {
      const errorData = e?.response?.data;
      const msg = typeof errorData === 'string' ? errorData : errorData?.message || e?.message || 'Failed to update subcategory';
      
      console.error('❌ Full error:', JSON.stringify(errorData));

      // Handle duplicate name specifically
      if (msg.toLowerCase().includes('already exists') || msg.toLowerCase().includes('duplicate')) {
        setNameError('This subcategory name already exists');
      } else {
          ...
          >
            ...
            <View style={styles.cardHeader}>
              <View style={styles.brandContainer}>
                ...
                <Text style={styles.cardTitle}>Edit Sub Category</Text>
              </View>
              ...
              <View style={styles.imageSection}>
                ...
                {imageError ? (
                  ...
                  <Text style={styles.formTip}>
                    Current photo above. Tap to change.
                  </Text>
                )}
              </View>
              ...
              <View style={styles.formFields}>
                ...
                >
                  {loading ? (
                    ...
                    <Text style={styles.submitBtnText}>Update Sub Category</Text>
                  )}
                </TouchableOpacity>
              </View>
            </View>
